...
@section('content')
        ...
        <div class="row">
            <div class="col-md-6 tx-left">
                <h1 class="tx-gray-800 tx-bold mg-b-10">
                    ...
                    <span class="menu-item-label">{{ __('List Permissions') }}</span>
                </h1>
            </div>
            <div class="col-md-6 tx-right">
                <button type="button" class="btn btn-teal mg-b-20 pd-r-20" data-toggle="modal" data-target="#create_permission_modal">
                    <i class="fa fa-plus mg-r-5"></i> {{ __('Add permission') }}
                </button>
            </div>
        </div>
                    ...
                    @foreach($permissions as $permission)
                        <tr>
                            ...
                            <td>
                                ...
                                &nbsp;
                                <a href="javascript:void(0)" data-id="{{$permission->id}}" class="btn btn-danger delete"  id="btn-delete" title="{{ __('Delete') }}">
    ...
    <!-- Create modal -->
    <div id="create_permission_modal" class="modal fade">
        <div class="modal-dialog modal-dialog-vertical-center" role="document">
            <div class="modal-content bd-0 tx-14">
                <div class="modal-header pd-y-20 pd-x-25">
                    <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">{{ __('Create Permission') }}</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('Close') }}">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body pd-25">
                    <form id="form_create" action="{{route('permissions.store')}}" method="POST">
                        {{ method_field("POST") }}
                        {{ csrf_field() }}
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="form-control-label tx-bold">{{ __('Key') }}</label>
    ...
    <!-- Edit modal -->
    <div id="update_permission_modal" class="modal fade">
        <div class="modal-dialog modal-dialog-vertical-center" role="document">
            <div class="modal-content bd-0 tx-14">
                <div class="modal-header pd-y-20 pd-x-25">
                    <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">{{ __('Update Permission') }}</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="{{ __('Close') }}">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body pd-25">
                    <form id="form_update" action="{{route('permissions.index')}}" method="POST">
                        {{ method_field("PUT") }}
                        {{ csrf_field() }}
                        <div class="row">
                            <div class="col-md-12">
    ...
    <div class="modal fade" tabindex="-1" id="delete_modal" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content bd-0">
                ...
                <div class="modal-footer">
                    <form action="{{route('permissions.index')}}" id="form_delete" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
...
@section('script')
    <script>
        ...
        $('td').on('click', '.delete', function (e) {
            $('#form_delete')[0].action = '{{ route('permissions.destroy', ['id' => '__id']) }}'.replace('__id', $(this).data('id'));            
        ...
        $(document).on('click', '#btn_edit', function() {
            ...
            form.find("#key").val('').removeClass('is-invalid');
            ...
            $.ajax({
                ...
                success : function (data) {
                    ...
                    if (data.type == 'error') {
                        ...
                        form[0].action = '{{ route('permissions.update', ['id' => '__id']) }}'.replace('__id', data.data['id']);
        ...
        function submitFormCreateOrUpdate(idForm) {
            ...
            if (key.length < 1 || !key.trim()) {
                ...
                form.find("#key").after('<p class="error text-danger">This field is required</p>');
                return false;
            }
